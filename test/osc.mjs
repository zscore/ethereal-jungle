...
const fresh = () => ({ tensionMix: 0, tensionManual: 0.4, brightnessMix: 0, brightnessManual: 0.7, wildness: 0.35, coupling: 0.6, seed: 1, lpf: 1, hpf: 0, echo: 0, crush: 0, space: 0, eqLow: 1, eqMid: 1, eqHigh: 1, gate: 0, drive: 0, roll: 0 });
...
console.log('address form');
{
  ...
  check(applyOscMessage(p, { address: '/jungle/lpf', args: [0.2] }) === 'lpf' && p.lpf === 0.2, 'perform rail is writable (D17)');
  check(applyOscMessage(p, { address: '/jungle/hpf', args: [0.4] }) === 'hpf' && p.hpf === 0.4, 'the two filter dials are independent (D20)');
  check(applyOscMessage(p, { address: '/jungle/filter', args: [0.2] }) === null, 'the retired bipolar filter is rejected');
