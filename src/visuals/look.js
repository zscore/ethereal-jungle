...
//
// Everything near the lens used to render SHARP, because the focal length idled
// at "off" and the depth-of-field pass was a no-op until a hand touched the
// rail. (K5's fronds at 2.8–3.4 units were the loudest case; D42 removed them,
// and the near-field dust and the trunks make the same argument.) The pizzaz
// doc listed this under *what is not done* and called it a decision rather than
// a bug, which was right: giving the world an aperture breaks F–J's promise
// that at rest the frame is indistinguishable from the one before the optics
// landed. It is made here deliberately. A forest at eye level has almost no
// depth of field — the leaf at your face is a smear and you cannot make it not
// be — and a frame where 3 units and 40 units are equally sharp is a diagram,
// not a look.
//
 ...
 * last leaf. It is what the world's resting aperture (`look`) is scaled by,
 * because an aperture is only honest where there is something close enough for
 * it to be about: trunks, undergrowth and near-field dust down among them.
 * K5's fronds used to be its second reader; D42 deleted them and the curve did
 * not move, because they were one item of the near content and not the whole of
 * it. Above the crowns there is nothing within 40 units of the camera and
...
//
// Retuned for the ascent (D39). The old table shrank the top band to a radius
