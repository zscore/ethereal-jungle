 ...
 *   the rail      (filter/echo/crush/space, D17) — the hand on the mixer,
 ...
 *   filter / space  move THE WORLD    (distance: defocus, fog, vignette)
 ...
 */
import { filterNeutral } from '../perform.js';

const clamp01 = (x) => (x < 0 ? 0 : x > 1 ? 1 : x);

/** The bipolar DJ filter, split into its two one-sided percepts. */
export function railSplit(filter = 0.5) {
  if (filterNeutral(filter)) return { lp: 0, hp: 0 };
  const x = clamp01(filter);
  return { lp: clamp01((0.5 - x) * 2), hp: clamp01((x - 0.5) * 2) };
}
...
export function look(params = {}, env = {}) {
  const { lp, hp } = railSplit(params.filter);
