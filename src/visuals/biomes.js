 ...
 * long shot.
 *
 * ONE FOREST, FOUR LEVELS (D39). The bands are no longer four family demos
...
// ---------- the forest itself: trunks and crowns (D39) ----------
...
function makeAir() {
  ...
  return {
    ...
    group: mesh,
    update(dt, env) {
      // D39 — the sphere no longer rides the brightness walk. It is 170 units
...
function makeMist(rng) {
  ...
  for (let i = 0; i < COUNT; i++) {
    ...
    // D39 — the profile is bimodal, because a rainforest's is. Most of the
  ...
  return {
    ...
    update(dt, env) {
      ...
      for (const c of cards) {
        ...
        // the band you are in. Tightened from 20 to 14 by D39: with the crowns
...
// ---------- light shafts through the crowns (proposal C1, re-hung by D39) ----------
