...
1. The mischief layer (theory §8): master-bus jokes — needs the master bus as
   an addressable target. Its visual twins are already free: the perform rail's
   twins (D19) prove the mechanism, and `look.js` is where the events would land.
2. The canopy ether on TSL compute particles (`docs/scene_plan.md` roadmap 1) —
   the CPU advection is behind a stable interface, so this is a swap.
